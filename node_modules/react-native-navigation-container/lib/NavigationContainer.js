...
import { NavigationContainer as RNavContainer, } from '@react-navigation/native';
 ...
 */
const usePromiseAll = (promises, cb) => useEffect(() => {
    (async () => {
...
const NavigationContainer = ({ assets, fonts, children, initialState, onStateChange, stickyNav, ...props }) => {
     ...
     */
    const stickyNavOnStateChange = (state) => AsyncStorage.setItem(NAVIGATION_STATE_KEY, JSON.stringify(state));
    if (!ready || !isNavigationReady) {
        return React.createElement(AppLoading, null);
    }
    return (React.createElement(RNavContainer, { onStateChange: stickyNav ? stickyNavOnStateChange : onStateChange, initialState: stickyNav ? stickyNavInitialState : initialState, ...props }, children));
};
